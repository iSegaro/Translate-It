  ...
  async _loadPanelManagerImpl() {
    ...
    try {
      if (panelSystem === 'side_panel' && capabilities.sidePanel) {
        const { ChromeSidePanelManager } = await import('../managers/sidepanel-chrome.js');
        return new ChromeSidePanelManager();
      } else {
        // Firefox sidebar or fallback
        const { FirefoxSidebarManager } = await import('../managers/sidebar-firefox.js');
  ...
  async _loadScreenCaptureManagerImpl() {
    ...
    try {
      if (capabilities.offscreen && browser === 'chrome') {
        const { OffscreenCaptureManager } = await import('../managers/capture-offscreen.js');
        return new OffscreenCaptureManager();
      } else {
        const { ContentScriptCaptureManager } = await import('../managers/capture-content.js');
  ...
  async preloadEssentialFeatures() {
    ...
    results.forEach((result, index) => {
      ...
      if (result.status === 'fulfilled') {
        ...
        console.error(`❌ Failed to load ${featureName} feature:`, result.reason);
      }
    });
  ...
  async loadAllListeners() {
    ...
    results.forEach((result, index) => {
      if (result.status === 'fulfilled') {
        ...
        console.error(`❌ Failed to load listener ${listenerNames[index]}:`, result.reason);
      }
    });
