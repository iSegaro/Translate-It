...
function regionComparisonResult(results = []) {
  return { results }
}
...
describe('RegionComparisonAnalyzer', () => {
  ...
  it('handles empty, failed, and single recognized outputs as not comparable', () => {
    const analyzer = new RegionComparisonAnalyzer()

    expect(analyzer.analyze(regionComparisonResult()).output).toEqual({ identical: false, comparable: false, uniqueOutputCount: 0 })
    expect(analyzer.analyze(regionComparisonResult([{ output: { status: 'failed' } }, recognized('one')])).output)
      .toEqual({ identical: false, comparable: false, uniqueOutputCount: 1 })
    expect(analyzer.analyze(regionComparisonResult([{ output: { status: 'failed' } }])).output)
  ...
  it('handles missing confidence, one confidence, missing latency, and partial evaluation', () => {
    ...
    expect(new RegionComparisonAnalyzer().analyze(regionComparisonResult([recognized('third')])).latency)
      .toEqual({ fastestMs: null, slowestMs: null, deltaMs: null })
  })

  it('does not freeze caller-owned result objects', () => {
    const input = regionComparisonResult([recognized('first', { confidence: 90, latencyMs: 10 })])
    const output = input.results[0].output

    new RegionComparisonAnalyzer().analyze(input)
