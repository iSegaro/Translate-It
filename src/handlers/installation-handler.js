...
async function detectLegacyMigration() {
  try {
    ...
    const hasLegacyData = (('API_KEY' in storage || 'TRANSLATION_API' in storage) && !hasVueMarkers) ||
...
async function performLegacyMigration(existingData) {
  try {
    ...
    return { success: true, migratedKeys: Object.keys(migratedData), migrationLog };
  } catch (error) {
...
async function migrateConfigSettings() {
  try {
    ...
    if (migrationStatus.isLegacyMigration) {
      logME('[InstallationHandler] Legacy migration detected - performing full migration');
    ...
    Object.keys(CONFIG).forEach((key) => {
      if (key in currentSettings && currentSettings[key] !== CONFIG[key]) {
        logME(`[InstallationHandler] Preserving user setting: ${key} = ${currentSettings[key]} (default: ${CONFIG[key]})`);
      }
    });

    if (newKeys.length > 0) {
      logME(`[InstallationHandler] Adding ${newKeys.length} new config keys:`, newKeys);
      ...
      logME('[InstallationHandler] No new config keys found, migration skipped');
    }
...
async function handleFreshInstallation() {
  ...
  if (hasExistingData) {
    logME('[InstallationHandler] Existing data found during fresh install - likely legacy migration');
    ...
    try {
      ...
      logME('[InstallationHandler] Failed to show migration notification:', error);
    }
  } else {
...
async function handleExtensionUpdate() {
  try {
    console.log('[InstallationHandler] Starting update notification creation...');
    ...
    console.log('[InstallationHandler] Notification details:', { appName, title, message });
    ...
    console.log('[InstallationHandler] Notification options:', notificationOptions);
    ...
    console.log('[InstallationHandler] Notification created with ID:', notificationId);
    logME("[InstallationHandler] Update notification created with browser-specific options.");
  } catch (e) {
    // This will now only catch unexpected errors, not the compatibility error.
    console.error("[InstallationHandler] Failed to create update notification:", e);
...
async function setupContextMenus() {
  try {
    ...
    const pageMenuTitle = (await getTranslationString('context_menu_translate_with_selection')) || 'Translate Element';
    console.log('[InstallationHandler] Creating context menu with title:', pageMenuTitle);
    ...
    console.error('[InstallationHandler] Failed to setup context menus:', error);
...
export async function handleInstallationEvent(details) {
  console.log(`[InstallationHandler] 🌟 Installation event triggered: ${details.reason}`);
