...
async function handleUpdateNotification() {
  try {
    ...
    logME('[NotificationHandler] Update notification handled - opened options page');
  } catch (error) {
...
async function handleMigrationNotification() {
  try {
    ...
    logME('[NotificationHandler] Migration notification handled - opened options page');
  } catch (error) {
    logME('[NotificationHandler] Error handling migration notification:', error);
  }
}
...
async function handleProviderChangeNotification() {
  try {
    ...
    logME('[NotificationHandler] Provider change notification handled - opened API settings');
  } catch (error) {
    logME('[NotificationHandler] Error handling provider change notification:', error);
  }
}
...
async function handleSiteExclusionNotification() {
  try {
    ...
    logME('[NotificationHandler] Site exclusion notification handled - opened advanced settings');
  } catch (error) {
    logME('[NotificationHandler] Error handling site exclusion notification:', error);
  }
}
...
async function handleSiteInclusionNotification() {
  try {
    ...
    logME('[NotificationHandler] Site inclusion notification handled - opened advanced settings');
  } catch (error) {
    logME('[NotificationHandler] Error handling site inclusion notification:', error);
  }
}
...
async function handleTranslationErrorNotification() {
  try {
    ...
    logME('[NotificationHandler] Translation error notification handled - opened API settings');
  } catch (error) {
    logME('[NotificationHandler] Error handling translation error notification:', error);
  }
}
...
async function handleSuccessNotification(notificationId) {
  try {
    logME(`[NotificationHandler] Success notification clicked: ${notificationId}`);
    
    // Just clear the notification
    await browser.notifications.clear(notificationId);
    
    logME(`[NotificationHandler] Success notification ${notificationId} cleared`);
  } catch (error) {
...
async function handleErrorNotification(notificationId) {
  try {
    logME(`[NotificationHandler] Error notification clicked: ${notificationId}`);
    ...
    logME(`[NotificationHandler] Error notification ${notificationId} handled - opened help page`);
  } catch (error) {
    ...
    switch (notificationId) {
      ...
      default:
        ...
        if (notificationId.includes('success')) {
          ...
          logME(`[NotificationHandler] Unknown notification: ${notificationId}, clearing`);
          await browser.notifications.clear(notificationId);
        }
        break;
    }
    
    logME(`[NotificationHandler] Notification ${notificationId} handled successfully`);
  } catch (error) {
    logME(`[NotificationHandler] Error handling notification ${notificationId}:`, error);
    throw error;
  }
}