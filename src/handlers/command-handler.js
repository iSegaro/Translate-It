    ...
    if (currentWindow) {
      ...
      try {
        ...
        try {
          ...
          logME('[CommandHandler] Failed to open popup or sidepanel:', { popupError, sidepanelError });
        }
      }
    }
  } catch (error) {
...
export async function handleCommandEvent(command, tab) {
  logME(`[CommandHandler] Command received: ${command}`, { tabId: tab?.id, url: tab?.url });
