...
export function useTTS() {
  ...
  const currentVoice = computed(() => {
    if (settings.value.voice) {
      return availableVoices.value.find(voice => voice.voiceURI === settings.value.voice)
    }
  ...
  const extractGender = (voiceName) => {
    ...
    const femaleNames = ['anna', 'emma', 'samantha', 'kate', 'sarah', 'victoria', 'zoe']
  ...
  const getDefaultVoice = () => {
    ...
    if (settings.value.preferredVoiceGender !== 'neutral') {
      const genderFiltered = preferredVoices.filter(voice => 
        voice.gender === settings.value.preferredVoiceGender
      )
  ...
  // Test functionality
  const testVoice = async (voiceURI, testText = 'Hello, this is a test of the text to speech system.') => {
    const voice = availableVoices.value.find(v => v.voiceURI === voiceURI)
    if (!voice) {
  ...
  const setLanguageFromText = (text) => {
    ...
    const bestVoice = findBestVoice(detectedLang, settings.value.preferredVoiceGender)
