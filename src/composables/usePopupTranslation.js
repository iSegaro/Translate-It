...
export function usePopupTranslation() {
  ...
  const triggerTranslation = async (sourceLang = null, targetLang = null) => {
    ...
    try {
      ...
      try {
        ...
        // Handle direct response if result is present
        let resultData = null;
        ...
        if (resultData && resultData.translatedText) {
          ...
          lastTranslation.value = {
            source: resultData.originalText,
            target: resultData.translatedText,
            provider: resultData.provider,
            timestamp: resultData.timestamp,
          };
          isTranslating.value = false;
          logger.info('Translation updated from direct response');
        } else if (resultData && resultData.success === false && resultData.error) {
          const errorMessage = resultData.error?.message || resultData.error?.type || resultData.error || "Translation failed";
  ...
  onMounted(() => {
    ...
    popupMessageListener = (message) => {
      ...
      // Robust extraction: handle result from message.result, message.data, or message.translatedText
      let resultData = null;
      if (message.result) {
        ...
        // Direct translation result (rare)
        resultData = message;
      }
      ...
      if (resultData && (resultData.translatedText || resultData.success === false)) {
        ...
        if (resultData.success === false && resultData.error) {
          logger.error('Translation error received', resultData.error);
          const errorMessage = resultData.error?.message || resultData.error?.type || resultData.error || "Translation failed";
          errorManager.handleError(errorMessage);
          translatedText.value = "";
          lastTranslation.value = null;
          ...
          logger.warn('Unexpected message data structure:', resultData);
          errorManager.handleError("Unexpected response format");
          translatedText.value = "";
        }
      } else {
        logger.debug('Message filtered out. Action:', message.action, 'Type:', message.type, 'Context:', message.context);
      }
    };
