...
export function useApiProvider() {
  ...
  const currentProviderData = computed(() => {
    console.log(
      "[useApiProvider] currentProviderData computed - currentProvider.value:",
      currentProvider.value,
    );
    console.log(
      "[useApiProvider] currentProviderData computed - availableProviders.value:",
      availableProviders.value,
    );
    const data =
      availableProviders.value.find((p) => p.id === currentProvider.value) ||
      null;
    console.log(
      "[useApiProvider] currentProviderData computed - result data:",
      data,
    );
    return data;
  });
  ...
  const loadAvailableProviders = () => {
    try {
      ...
      console.log(
        `[useApiProvider] Loaded ${availableProviders.value.length} available providers`,
      );
  ...
  const loadCurrentProvider = async () => {
    try {
      ...
      currentProvider.value =
        settingsStore.settings.TRANSLATION_API || "google";
      console.log(
        "[useApiProvider] loadCurrentProvider - currentProvider.value set to:",
        currentProvider.value,
      );
      ...
      if (!providerExists) {
        console.log(
          `[useApiProvider] Provider ${currentProvider.value} not available, using fallback: google`,
        );
  ...
  const selectProvider = async (providerId) => {
    if (!providerId || providerId === currentProvider.value) {
      console.log(
        "[useApiProvider] selectProvider - Provider already selected or invalid ID:",
        providerId,
      );
      return false;
    }
    ...
    try {
      ...
      console.log(
        "[useApiProvider] selectProvider - currentProvider.value updated to:",
        currentProvider.value,
      );
  ...
  const createProviderItems = async () => {
    ...
    for (const provider of availableProviders.value) {
      const iconUrl = await getProviderIconUrl(
        `icons/api-providers/${provider.icon}`,
      );
      items.push({
        id: provider.id,
        name: provider.name,
  ...
  watch(
    ...
    (newProvider) => {
      if (newProvider && newProvider !== currentProvider.value) {
        ...
        console.log(
          `[useApiProvider] Provider updated from settings store: ${newProvider}`,
        );
      }
    },
  );
  ...
  const setupStorageListener = async () => {
    try {
          ...
          (changes, areaName) => {
            if (areaName === "local" && changes.TRANSLATION_API) {
              ...
              if (newProvider && newProvider !== currentProvider.value) {
                ...
                console.log(
                  `[useApiProvider] Provider synced from storage: ${newProvider}`,
                );
              }
            }
          },
      ...
      console.error(
        "[useApiProvider] Error setting up storage listener:",
        error,
      );
    }
  };
