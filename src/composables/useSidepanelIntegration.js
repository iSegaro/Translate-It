...
const toggleInlineToolbarVisibility = (container) => {
  ...
  const hasContent = textElement && textElement.value && textElement.value.trim().length > 0
...
export function useSidepanelIntegration() {
  ...
  const initializeLanguages = async () => {
    try {
      ...
      console.error('[useSidepanelIntegration] Error initializing languages:', error)
    }
  }
  ...
  const setupEventListeners = () => {
    ...
    if (elements.value.translationForm) {
      elements.value.translationForm.addEventListener('submit', async (event) => {
    ...
    if (elements.value.swapLanguagesBtn) {
      elements.value.swapLanguagesBtn.addEventListener('click', async () => {
        ...
        if (success) {
          // Update DOM selects
          elements.value.sourceLanguageInput.value = translation.sourceLanguage.value
          elements.value.targetLanguageInput.value = translation.targetLanguage.value
    ...
    if (elements.value.copyTargetBtn) {
      elements.value.copyTargetBtn.addEventListener('click', () => {
        ...
        const textToCopy = resultElement?.dataset?.originalMarkdown || 
    ...
    if (elements.value.clearFieldsBtn) {
      elements.value.clearFieldsBtn.addEventListener('click', () => {
        ...
        toggleInlineToolbarVisibility(elements.value.sourceText.parentElement)
        toggleInlineToolbarVisibility(elements.value.translationResult.parentElement)
      })
    }
    ...
    if (elements.value.selectElementBtn) {
      elements.value.selectElementBtn.addEventListener('click', async () => {
        try {
          ...
          console.error('[useSidepanelIntegration] Select element mode failed:', error)
        }
      })
    }
    ...
    if (elements.value.revertActionBtn) {
      elements.value.revertActionBtn.addEventListener('click', () => {
        browserAPI.safeSendMessage({ action: 'revertTranslation' })
          .catch(err => console.warn('[useSidepanelIntegration] Revert failed:', err))
      })
    }
  ...
  const performTranslation = async () => {
    ...
    translation.sourceText.value = elements.value.sourceText?.value || ''
    translation.sourceLanguage.value = elements.value.sourceLanguageInput?.value || AUTO_DETECT_VALUE
    translation.targetLanguage.value = elements.value.targetLanguageInput?.value || 'English'

    // Perform translation using Vue composable
    const success = await translation.triggerTranslation(elements.value.translationResult)
  ...
  const setupMessageListener = async () => {
    try {
      ...
      console.error('[useSidepanelIntegration] Message listener setup failed:', error)
    }
  }
  ...
  const initialize = async () => {
    try {
      ...
      integrationError.value = error.message || 'Failed to initialize sidepanel'
    }
  }
