...
export function useTranslation() {
  ...
  const handleTranslationResponse = async (
    response,
    textToTranslate,
  ...
  const swapLanguages = async () => {
    ...
    if (sourceCode === AUTO_DETECT_VALUE) {
      try {
        ...
        console.error(
          "[useTranslation] Failed to load source language from settings",
          err,
        );
    ...
    if (targetCode === AUTO_DETECT_VALUE) {
      try {
        ...
        console.error(
          "[useTranslation] Failed to load target language from settings",
          err,
        );
    ...
    if (
      ...
      console.warn(
        "[useTranslation] Cannot swap - invalid language selection.",
        {
          resolvedSourceCode,
          resolvedTargetCode,
        },
    ...
    try {
      ...
      if (settings.lastTranslation) {
        const {
          ...
          sourceLanguage: lastSourceLang,
          targetLanguage: lastTargetLang,
