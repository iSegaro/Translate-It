...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const isSelectionModeActive = await getTranslateWithSelectElementAsync();
  ...
  if (isSelectionModeActive) {
    // Attach mouseover event to the popup container
    ...
    popupContainer.addEventListener("mouseleave", () => {
      ...
      chrome.storage.local.get(["selectElementState"], (result) => {
        ...
        if (result.selectElementState) {
          ...
          logME(
            "[Popup]: Mouse left, but select mode not active. Not closing."
          );
        }
      });
    });
  ...
  logME(
    "[Popup]: Initial setup complete, selection mode active:",
