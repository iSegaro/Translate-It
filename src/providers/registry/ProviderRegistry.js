  ...
  static getAvailableProviders(browserName = null) {
    ...
    return this.providers.filter((provider) => {
      ...
      if (!provider.requirements.browsers.includes(browser)) {
        logME(
          `[ProviderRegistry] Provider ${provider.id} not compatible with ${browser}`
        );
        return false;
      }
      ...
      if (provider.requirements.minVersion && browser === "chrome") {
        const isVersionSupported = this._checkChromeVersion(
          provider.requirements.minVersion
        );
        if (!isVersionSupported) {
          logME(
            `[ProviderRegistry] Provider ${provider.id} requires Chrome ${provider.requirements.minVersion}+`
          );
          return false;
        }
      }
      ...
      if (provider.requirements.features.length > 0) {
        ...
        if (!hasFeatures) {
          logME(
            `[ProviderRegistry] Provider ${provider.id} missing required features: ${provider.requirements.features.join(", ")}`
          );
          return false;
        }
      }
