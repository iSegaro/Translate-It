...
async function migrateConfigSettings() {
  try {
    ...
    if (migrationStatus.isLegacyMigration) {
      logME(
        "[onInstalled] Legacy migration detected - performing full migration"
      );
    ...
    Object.keys(CONFIG).forEach((key) => {
      if (key in currentSettings && currentSettings[key] !== CONFIG[key]) {
        ...
        logME(
          `[onInstalled] Preserving user setting: ${key} = ${currentSettings[key]} (default: ${CONFIG[key]})`
        );
      }
    });
...
class InstallationListener extends BaseListener {
  ...
  async handleInstallation(details) {
    ...
    await setupContextMenus(browser);
    ...
    try {
      ...
      logME(
        "[onInstalled] Config migration failed, but continuing with other setup:",
        error
      );
    }
  ...
  async handleFreshInstallation() {
    ...
    if (hasExistingData) {
      logME(
        "[onInstalled] Existing data found during fresh install - likely legacy migration"
      );
