...
async function safeSendMessage(Browser, tabId, message) {
  try {
    ...
    if (errorType === ErrorTypes.EXTENSION_CONTEXT_INVALIDATED) {
      logME(
        "[onMessage] Extension context invalidated - content script connection lost"
      );
...
class MessageListener extends BaseListener {
  ...
  async handleSimpleTTSSpeak(message, sendResponse) {
    try {
      logME("[onMessage] Simple TTS speak request:", {
  ...
  async handleCachedTTSSpeak(message, sendResponse) {
    try {
      logME("[onMessage] Cached TTS speak request:", {
        text: message.text?.substring(0, 30),
        lang: message.lang,
      });
  ...
  async handlePlayCachedAudio(message, sendResponse) {
    try {
      logME(
        "[onMessage] Cached audio playback request:",
        message.audioData?.length,
        "bytes"
      );
      ...
      try {
        ...
        if (typeof ttsManager.recreateOffscreenIfNeeded === "function") {
          logME(
            "[onMessage] Ensuring offscreen is ready for cached audio playback"
          );
          const isReady = await ttsManager.recreateOffscreenIfNeeded();
          if (!isReady) {
            throw new Error(
              "Offscreen document not available and recreation failed"
            );
          }
        }
      ...
      try {
        if (typeof chrome !== "undefined" && chrome.runtime) {
          ...
          if (response && response.success) {
            sendResponse({ success: true });
            logME(
              "[onMessage] Cached audio playback completed via direct offscreen"
            );
            return;
          } else {
            throw new Error(
              response?.error || "Direct offscreen playback failed"
            );
          }
        } else {
          throw new Error(
            "Chrome runtime not available for offscreen playback"
          );
        }
      } catch (directError) {
        lastError = directError;
        logME(
          "[onMessage] Direct offscreen playback failed:",
          directError.message
        );
      }
  ...
  handleMessage(message, sender, sendResponse) {
    ...
    if (message.forwardedFromOffscreen) {
      ...
      return undefined; // Changed to undefined to allow other listeners to handle it
    }
    ...
    try {
      switch (action) {
        ...
        case "getSelectedText":
          ...
          handleGetSelectedText(message, sender, sendResponse, (tabId, msg) =>
            safeSendMessage(this.browser, tabId, msg)
          );
          return true;
        ...
        case "fetchTranslation":
          ...
          const safeSendResponse = (response) => {
            if (responseAlreadySent) {
              logME(
                "[onMessage] WARNING: sendResponse already called, ignoring:",
                response
              );
              return;
            }
          ...
          responseTimer = setTimeout(() => {
            if (!responseAlreadySent) {
              logME(
                "[onMessage] Translation timeout, sending default error response"
              );
              safeSendResponse({
                success: false,
          ...
          handleFetchTranslation(
            ...
            logME(
              "[onMessage] handleFetchTranslation promise rejected:",
              error
            );
            if (!responseAlreadySent) {
              safeSendResponse({
        ...
        case "previewConfirmed":
          handlePreviewConfirmed(message, sender, sendResponse);
          return true;
        ...
        case "previewRetry":
          handlePreviewRetry(message, sender, sendResponse);
          return true;
        ...
        case "show_os_notification": {
          ...
          if (this.browser.notifications && msg) {
            this.browser.notifications
              .create({
                type: "basic",
                iconUrl: this.browser.runtime.getURL(
                  "icons/extension_icon_128.png"
                ), // Use a specific size
        ...
        default:
          ...
          return undefined; // Allow other listeners to handle it
      }
    } catch (err) {
