...
/**
 * Canonical V3 BlockGroup segment marker, as generated by
 * BlockGroupReconstructor.injectMarkers: @@TI_SEG_<entropy>_<sessionId>_<n\d>@@.
 *
 * The reconstructor's own splitter is whitespace-tolerant inside markers
 * (/@@\s*TI\s*_\s*SEG\s*_.../ ); this extractor mirrors that tolerance and then
 * collapses internal whitespace so whitespace-only rendering differences never
 * produce a false rejection. Only well-formed generator tokens (ending in a
 * segment id n\d+) are counted; broken bare tokens such as @@TI_SEG_@@ are
 * ignored identically to the reconstructor, which cannot split on them.
 *
 * NOTE: this does NOT import DOM/reconstructor code. It extracts marker
 * tokens from already-materialized source and translated strings, which is
 * safe to run in the background/service-worker layer.
 */
const V3_SEGMENT_MARKER_REGEX = /@@\s*TI\s*_\s*SEG\s*_\s*[^@]*?\s*@@/g;

function extractV3SegmentMarkers(text) {
  if (typeof text !== 'string') return [];
  const markers = [];
  const re = new RegExp(V3_SEGMENT_MARKER_REGEX.source, V3_SEGMENT_MARKER_REGEX.flags);
  let match;
  while ((match = re.exec(text)) !== null) {
    const normalized = match[0].replace(/\s+/g, '');
    if (/^@@TI_SEG_\w+_n\d+@@$/.test(normalized)) {
...
/**
 * Pre-stream V3 BlockGroup marker-contract check.
 *
 * @param {string} sourceText - the source `t` exactly as injected by injectMarkers
 * @param {string} translatedText - the AI-translated text
 * @returns {null|{reason:string, expectedMarkerCount:number, actualMarkerCount:number}}
 *   null when valid (or when the source carries no V3 markers, i.e. V2/Google/plain),
 *   otherwise a structured violation describing the mismatch.
 */
function validateV3MarkerContract(sourceText, translatedText) {
  const sourceMarkers = extractV3SegmentMarkers(sourceText);
  if (sourceMarkers.length === 0) return null;
  const translatedMarkers = extractV3SegmentMarkers(translatedText || '');
  if (translatedMarkers.length !== sourceMarkers.length) {
    return { reason: 'MARKER_COUNT_MISMATCH', expectedMarkerCount: sourceMarkers.length, actualMarkerCount: translatedMarkers.length };
  }
  for (let i = 0; i < sourceMarkers.length; i++) {
    if (translatedMarkers[i] !== sourceMarkers[i]) {
      return { reason: 'MARKER_SEQUENCE_MISMATCH', expectedMarkerCount: sourceMarkers.length, actualMarkerCount: translatedMarkers.length };
    }
  }
  return null;
}
            ...
            if (result?.isV3Fragment === true) {
             ...
             if (orderedFragments.some(fragment => !fragment)) {
              ...
              const sourceText = orderedFragments
                ...
                .join('');
              const violation = validateV3MarkerContract(sourceText, translatedText);
              if (violation) {
                ...
                appendTranslationDiagnostic(executionContext, {
                  ...
                  expectedMarkerCount: violation.expectedMarkerCount,
                  actualMarkerCount: violation.actualMarkerCount,
                  reason: violation.reason,
                });
                const err = new Error(`V3 marker contract violation for fragmented parent ${parentIdStr}: ${violation.reason} (expected ${violation.expectedMarkerCount}, received ${violation.actualMarkerCount})`);
                err.isFatal = true;
                err.type = ErrorTypes.VALIDATION;
                throw err;
              }
    ...
    return originalBatch.map((item, idx) => {
      ...
      if (typeof item === 'object' && item !== null) {
        ...
        if (!isFragment && typeof sourceText === 'string' && sourceText.includes('@@TI_SEG_')) {
          const violation = validateV3MarkerContract(sourceText, translatedContent);
          if (violation) {
            ...
            const err = new Error(`V3 marker contract violation for block ${blockId}: ${violation.reason} (expected ${violation.expectedMarkerCount}, received ${violation.actualMarkerCount})`);
            err.isFatal = true;
            err.type = ErrorTypes.VALIDATION;
            throw err;
          }
