...
import {
  CONFIG,
  getRequestyApiKeysAsync,
  getRequestyApiModelAsync,
...
const logger = getScopedLogger(LOG_COMPONENTS.PROVIDERS, 'Requesty');

export class RequestyProvider extends BaseAIProvider {
  static type = "ai";
  static description = "Requesty Multi-Model API";
  static displayName = "Requesty";

  constructor() {
    super(ProviderNames.REQUESTY);
    this.providerSettingKey = 'REQUESTY_API_KEY';
  }

  /**
   * Normalizes one raw Requesty response into one completion record at the
   ...
   * @private
   */
  _recordRequestyCompletion(data, executionContext) {
  ...
  /**
   * Internal implementation of the Requesty API call.
   * @protected
   */
  async _callAI(systemPrompt, userText, options = {}) {
    ...
    const turnNumber = conversationParticipates
      ...
      : 1;
    logger.info(`[Requesty] Model: ${model || 'default'}${sessionId ? ` (Session: ${sessionId.substring(0, 15)}..., Turn: ${turnNumber})` : ''}`);
    ...
    const fetchOptions = {
      ...
      body: JSON.stringify({
        model: model || CONFIG.REQUESTY_API_MODEL,
        messages: messages,
        max_tokens: 4096,
    ...
    const result = await this._executeRequest({
      ...
      extractResponse: (data) => {
        ...
        if (parsed?.error) {
          ...
          const errorMsg = errorInfo.message || errorInfo.metadata?.raw || (typeof errorInfo === 'string' ? errorInfo : 'Unknown Requesty Error');
          throw new Error(`API_ERROR: ${errorMsg}`);
        }

        // Error responses carry the error object as the ONLY field
        if (parsed && Object.keys(parsed).length === 1 && parsed.error) {
           throw new Error(`API_ERROR: ${parsed.error.message || 'Unknown Requesty Error'}`);
        }

        this._recordRequestyCompletion(parsed, executionContext);
