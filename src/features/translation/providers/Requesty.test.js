...
import { CONFIG, getRequestyApiModelAsync } from '@/shared/config/config.js';
...
vi.mock('@/shared/config/config.js', async (importOriginal) => {
  ...
  return {
    ...actual,
    getRequestyApiKeysAsync: vi.fn().mockResolvedValue(['test-key']),
    getRequestyApiModelAsync: vi.fn().mockResolvedValue('openai/gpt-4o-mini'),
  };
});
...
const REQUESTY_RAW_RESPONSE_FIXTURES = Object.freeze({
  metadataRichResponse: Object.freeze({
    ...
    choices: [{
      ...
      message: { content: 'Requesty Metadata Result' },
    }],
    usage: {
      prompt_tokens: 1017,
  ...
  metadataAbsentResponse: Object.freeze({
    choices: [{
      finish_reason: 'stop',
      message: { content: 'Requesty Minimal Result' },
    }],
  }),
  unknownTerminationResponse: Object.freeze({
    choices: [{
      finish_reason: 'repository_test_unknown',
      message: { content: 'Requesty Unknown Result' },
    }],
  }),
  objectResponse: Object.freeze({
    choices: [{ message: { content: 'Requesty Result' } }],
  }),
  stringifiedResponse: JSON.stringify({
    choices: [{ message: { content: 'Requesty String Result' } }],
  }),
...
describe('RequestyProvider Error Handling', () => {
  let provider;
  ...
  it('should handle successful translation', async () => {
    proxyManager.fetch.mockResolvedValue({
      ...
      json: () => Promise.resolve(REQUESTY_RAW_RESPONSE_FIXTURES.objectResponse),
      clone: function() { return this; }
    });

    const result = await provider._callAI('system', 'Hello World');
    expect(result).toBe('Requesty Result');
  });

  it.each(['openai/gpt-4o-mini', 'claude-haiku-4-5'])('preserves curated model payload for %s', async (model) => {
    getRequestyApiModelAsync.mockResolvedValue(model);
    ...
    expect(executeRequest.mock.calls[0][0].url).toBe(CONFIG.REQUESTY_API_URL);
  ...
  it('uses CONFIG default for missing text model selection', async () => {
    getRequestyApiModelAsync.mockResolvedValue(undefined);
    const executeRequest = vi.spyOn(provider, '_executeRequest').mockResolvedValue('translated');

    await provider._callAI('system', 'source');

    const payload = JSON.parse(executeRequest.mock.calls[0][0].fetchOptions.body);
    expect(payload.model).toBe(CONFIG.REQUESTY_API_MODEL);
  });

  it('records normalized completion metadata from the confirmed response shape', async () => {
    const operation = createTranslationOperation('requesty-completion');
    proxyManager.fetch.mockResolvedValue({
      ok: true,
      status: 200,
      headers: new Map([['content-type', 'application/json']]),
      json: () => Promise.resolve(REQUESTY_RAW_RESPONSE_FIXTURES.metadataRichResponse),
      clone: function() { return this; }
    });
    ...
    expect(result).toBe('Requesty Metadata Result');
    expect(record).toEqual({
      provider: 'Requesty',
      model: 'nvidia/test-model',
      termination: CompletionTermination.NORMAL,
      responseId: 'gen-requesty-1',
      usage: {
        inputTokens: 1017,
        outputTokens: 318,
  ...
  it('records absent optional metadata as null without changing text extraction', async () => {
    const operation = createTranslationOperation('requesty-missing-metadata');
    proxyManager.fetch.mockResolvedValue({
      ok: true,
      status: 200,
      headers: new Map([['content-type', 'application/json']]),
      json: () => Promise.resolve(REQUESTY_RAW_RESPONSE_FIXTURES.metadataAbsentResponse),
      clone: function() { return this; }
    });
    ...
    expect(result).toBe('Requesty Minimal Result');
  ...
  it('normalizes an unrecognized finish reason to UNKNOWN', async () => {
    const operation = createTranslationOperation('requesty-unknown-termination');
    proxyManager.fetch.mockResolvedValue({
      ok: true,
      status: 200,
      headers: new Map([['content-type', 'application/json']]),
      json: () => Promise.resolve(REQUESTY_RAW_RESPONSE_FIXTURES.unknownTerminationResponse),
      clone: function() { return this; }
    });

    await expect(provider._callAI('system', 'text', { executionContext: { operation } }))
      .resolves.toBe('Requesty Unknown Result');
  ...
  it('is safe without execution context', async () => {
    ...
    await expect(provider._callAI('system', 'text')).resolves.toBe('Requesty Metadata Result');
  });

  it('records one ordered completion per physical response', async () => {
    const operation = createTranslationOperation('requesty-multiple');
    const first = { ...REQUESTY_RAW_RESPONSE_FIXTURES.metadataRichResponse, id: 'response-1' };
    const second = { ...REQUESTY_RAW_RESPONSE_FIXTURES.metadataRichResponse, id: 'response-2' };
    proxyManager.fetch
  ...
  it('extracts the currently supported stringified response shape', async () => {
    proxyManager.fetch.mockResolvedValue({
      ...
      status: 200,
      headers: new Map([['content-type', 'application/json']]),
      json: () => Promise.resolve(REQUESTY_RAW_RESPONSE_FIXTURES.stringifiedResponse),
      clone: function() { return this; }
    });

    await expect(provider._callAI('system', 'Hello World')).resolves.toBe('Requesty String Result');
  });
  ...
  it('should detect API_ERROR wrapped in 200 OK response', async () => {
    proxyManager.fetch.mockResolvedValue({
      ...
      status: 200,
      headers: new Map([['content-type', 'application/json']]),
      json: () => Promise.resolve(REQUESTY_RAW_RESPONSE_FIXTURES.errorEnvelope),
      clone: function() { return this; }
    });
  ...
  it('should detect error when it is the only field', async () => {
    proxyManager.fetch.mockResolvedValue({
      ok: true,
      status: 200,
      headers: new Map([['content-type', 'application/json']]),
      json: () => Promise.resolve(REQUESTY_RAW_RESPONSE_FIXTURES.singleFieldErrorEnvelope),
      clone: function() { return this; }
    });
