...
vi.mock('@/shared/config/config.js', async (importOriginal) => ({
  ...
  getRequestyApiKeysAsync: vi.fn().mockResolvedValue(['key-1', 'key-2']),
...
describe('Requesty Internal Integration (Failover & Headers)', () => {
  let provider;
  ...
  it('should include Requesty auth and attribution headers and return successful result', async () => {
    ...
    // Verify auth and attribution headers
  ...
  it('should failover to second key when first key is unauthorized', async () => {
    ...
    expect(ApiKeyManager.promoteKey).toHaveBeenCalledWith('REQUESTY_API_KEY', 'key-2');
  });
