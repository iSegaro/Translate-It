...
describe('PdfExportCollector', () => {
  ...
  it('collects only translated blocks', async () => {
    setupPageSession(1, [
  ...
  it('sorts blocks by page then reading order', async () => {
    setupPageSession(2, [
  ...
  it('getExportStats computes correct metrics from translation states', () => {
    setTranslated('b1', 'Hola')
    setTranslated('b2', 'Mundo')
    setError('b3')
    ...
    expect(stats.translatedCount).toBe(2)
  ...
  describe('collectSpatialBlocks', () => {
    it('includes geometry and skips untranslated blocks', async () => {
      session.pageMetrics = [
      ...
      const collector = new PdfExportCollector(session)
      const pages = await collector.collectSpatialBlocks()
    ...
    it('includes canvasDataUrl when provided', async () => {
      session.pageMetrics = [
      ...
      const collector = new PdfExportCollector(session)
      const pages = await collector.collectSpatialBlocks(canvasUrls)
