...
describe("live-caption background controller", () => {
  ...
  it("starts browser speech STT once on start and restarts it on resume", async () => {
    const sttCoordinator = {
      startSession: vi.fn().mockResolvedValue({ state: "transcribing" }),
      pauseSession: vi.fn().mockResolvedValue({ state: "ready" }),
      stopSession: vi.fn().mockResolvedValue({ state: "ready" }),
      abortSession: vi.fn().mockResolvedValue({ state: "ready" }),
    };
    const controller = createController({ sttCoordinator });
    ...
    expect(startResponse.success).toBe(true);
    expect(sttCoordinator.startSession).toHaveBeenCalledTimes(1);
    ...
    expect(sttCoordinator.pauseSession).toHaveBeenCalledTimes(1);
    ...
    expect(sttCoordinator.startSession).toHaveBeenCalledTimes(2);
  });

  it("fails closed once when Browser Speech startSession fails after start runtime is active", async () => {
    const controller = createController();
    controller.sttCoordinator.startSession = vi.fn().mockRejectedValue(new Error("browser speech start failed"));
    controller.sttCoordinator.stopSession = vi.fn().mockResolvedValue({ state: "ready" });
    controller.captureCoordinator.stopRuntime = vi.fn();
    controller.offscreenBridge.requestRuntimeStop = vi.fn().mockResolvedValue({ ok: true });
    controller.sessionManager.failClosedCleanup = vi.fn();
    ...
    expect(response.action).toBe(LIVE_CAPTION_RUNTIME_ACTIONS.START);
    expect(response.status).toBe(LIVE_CAPTION_RUNTIME_RESPONSE_STATUSES.FAIL_CLOSED);
    expect(response.error.reason).toBe("provider_error");
    expect(controller.sttCoordinator.startSession).toHaveBeenCalledTimes(1);
    expect(controller.sttCoordinator.stopSession).toHaveBeenCalledTimes(1);
    expect(controller.captureCoordinator.stopRuntime).toHaveBeenCalledTimes(1);
    expect(controller.offscreenBridge.requestRuntimeStop).toHaveBeenCalledTimes(1);
    expect(controller.sessionManager.failClosedCleanup).toHaveBeenCalledTimes(1);
    expect(controller.sttCoordinator.stopSession.mock.invocationCallOrder[0]).toBeLessThan(
      controller.captureCoordinator.stopRuntime.mock.invocationCallOrder[0],
    );
    expect(controller.captureCoordinator.stopRuntime.mock.invocationCallOrder[0]).toBeLessThan(
      controller.offscreenBridge.requestRuntimeStop.mock.invocationCallOrder[0],
    );
    expect(controller.offscreenBridge.requestRuntimeStop.mock.invocationCallOrder[0]).toBeLessThan(
      controller.sessionManager.failClosedCleanup.mock.invocationCallOrder[0],
    );
  });

  it("fails closed once when Browser Speech startSession fails after resume runtime is active", async () => {
    const controller = createController();
    controller.sttCoordinator.startSession = vi.fn().mockResolvedValueOnce({ state: "transcribing" }).mockRejectedValueOnce(new Error("browser speech resume failed"));
    controller.sttCoordinator.stopSession = vi.fn().mockResolvedValue({ state: "ready" });
    controller.captureCoordinator.stopRuntime = vi.fn();
    controller.offscreenBridge.requestRuntimeStop = vi.fn().mockResolvedValue({ ok: true });
    controller.sessionManager.failClosedCleanup = vi.fn();
    ...
    expect(resumeResponse.success).toBe(false);
    expect(resumeResponse.action).toBe(LIVE_CAPTION_RUNTIME_ACTIONS.RESUME);
    expect(resumeResponse.status).toBe(LIVE_CAPTION_RUNTIME_RESPONSE_STATUSES.FAIL_CLOSED);
    expect(resumeResponse.error.reason).toBe("provider_error");
    expect(controller.sttCoordinator.startSession).toHaveBeenCalledTimes(2);
    expect(controller.sttCoordinator.stopSession).toHaveBeenCalledTimes(1);
    expect(controller.captureCoordinator.stopRuntime).toHaveBeenCalledTimes(1);
    expect(controller.offscreenBridge.requestRuntimeStop).toHaveBeenCalledTimes(1);
    expect(controller.sessionManager.failClosedCleanup).toHaveBeenCalledTimes(1);
