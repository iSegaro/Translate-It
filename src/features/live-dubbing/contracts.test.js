...
describe('live dubbing Stage 2 contracts', () => {
  ...
  it('exposes the required operation timeout contract', () => {
    expect(LIVE_DUBBING_TIMEOUTS).toEqual({
      START: 30_000,
      STOP: 10_000,
      STATUS: 5_000,
      SETUP: 10_000,
      VALIDATE_CREDENTIAL: 30_000,
    });
    ...
    expect(LIVE_DUBBING_ACTION_TIMEOUTS[LIVE_DUBBING_ACTIONS.VALIDATE_CREDENTIAL])
      .toBe(LIVE_DUBBING_TIMEOUTS.VALIDATE_CREDENTIAL);
    expect(LIVE_DUBBING_ACTION_TIMEOUTS[LIVE_DUBBING_ACTIONS.REQUEST_PROVIDER_BOOTSTRAP])
      .toBe(LIVE_DUBBING_TIMEOUTS.SETUP);
    expect(LIVE_DUBBING_ACTION_TIMEOUTS[LIVE_DUBBING_ACTIONS.PREPARE])
      .toBe(LIVE_DUBBING_TIMEOUTS.SETUP);
    expect(LIVE_DUBBING_ACTION_TIMEOUTS[LIVE_DUBBING_ACTIONS.CONSUME])
      .toBe(LIVE_DUBBING_TIMEOUTS.SETUP);
    expect(LIVE_DUBBING_ACTION_TIMEOUTS[LIVE_DUBBING_ACTIONS.CONNECT_PROVIDER])
      .toBe(LIVE_DUBBING_TIMEOUTS.SETUP);
    expect(LIVE_DUBBING_ACTION_TIMEOUTS[LIVE_DUBBING_ACTIONS.DISPOSE])
      .toBe(LIVE_DUBBING_TIMEOUTS.SETUP);
