 ...
 *
 * It owns protocol framing only. Audio capture, decoding, playback, retries,
 * and transcript handling intentionally stay outside this feature-local class.
 */
export class GeminiLiveTranslationClient {
  ...
  _parseServerContent(serverContent, socket, generation) {
    ...
    for (const audio of audioParts) {
      this._emit(this.onAudio, audio);
      this._emitEvent({ type: 'audio', mimeType: audio.mimeType });
    }
...
export default GeminiLiveTranslationClient;
