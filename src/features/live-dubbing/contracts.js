...
const supportedMeasurementVariants = new Set(['100ms', '40ms']);
const supportedMeasurementFrameSamples = new Set([1_600, 640]);

function readMeasurementField(source, field) {
  try {
    return source[field];
  } catch {
    return undefined;
...
function safeMeasurementCount(value) {
  return Number.isSafeInteger(value) && value >= 0 ? value : 0;
}

function safeMeasurementDuration(value) {
  return typeof value === 'number' && Number.isFinite(value) && value >= 0 ? value : 0;
}

function safeMeasurementNullableDuration(value) {
  return typeof value === 'number' && Number.isFinite(value) && value >= 0 ? value : null;
}

/**
 * Create the exact persisted measurement-summary DTO. It is intentionally a
 * fresh, scalar-only object with an allowlisted field set so session and tab
 * identity, stream IDs, credentials, URLs, PCM, transcripts, provider
 * payloads, errors, and nested objects cannot cross into background storage
 * or logs. Returns null when the candidate is not a measurable summary.
 * @param {unknown} value
 * @returns {{variant: string, runDurationMs: number|null, inputFrameSamples: number, inputFrameDurationMs: number, inputFrames: number, inputSentFrames: number, inputPendingFrames: number, inputQueuePeakFrames: number, inputQueuePeakMs: number, inputDroppedDurationMs: number, inputBackpressureEvents: number, sendFailures: number, wsBufferedAmountPeak: number, translatedAudioChunks: number, outputQueuePeakMs: number, outputSafetyDrops: number, underruns: number, underrunSamples: number, underrunDurationMs: number, interruptions: number, firstInputToFirstTranslatedAudioMs: number|null, firstInputToPlaybackAcceptedMs: number|null, terminalCategory: string|null, playbackAccepted: boolean}|null}
 */
export function createLiveDubbingMeasurementSummary(value = {}) {
  const source = value && typeof value === 'object' && !(value instanceof Error) && !Array.isArray(value)
    ? value
    : null;
  if (!source) return null;
  const variant = readMeasurementField(source, 'variant');
  if (!supportedMeasurementVariants.has(variant)) return null;
  const inputFrameSamples = readMeasurementField(source, 'inputFrameSamples');
  if (!supportedMeasurementFrameSamples.has(inputFrameSamples)) return null;
  const terminalCategory = readMeasurementField(source, 'terminalCategory');
  return {
    variant,
    runDurationMs: safeMeasurementNullableDuration(readMeasurementField(source, 'runDurationMs')),
    inputFrameSamples,
    inputFrameDurationMs: safeMeasurementDuration(readMeasurementField(source, 'inputFrameDurationMs')),
    inputFrames: safeMeasurementCount(readMeasurementField(source, 'inputFrames')),
    inputSentFrames: safeMeasurementCount(readMeasurementField(source, 'inputSentFrames')),
    inputPendingFrames: safeMeasurementCount(readMeasurementField(source, 'inputPendingFrames')),
    inputQueuePeakFrames: safeMeasurementCount(readMeasurementField(source, 'inputQueuePeakFrames')),
    inputQueuePeakMs: safeMeasurementDuration(readMeasurementField(source, 'inputQueuePeakMs')),
    inputDroppedDurationMs: safeMeasurementDuration(readMeasurementField(source, 'inputDroppedDurationMs')),
    inputBackpressureEvents: safeMeasurementCount(readMeasurementField(source, 'inputBackpressureEvents')),
    sendFailures: safeMeasurementCount(readMeasurementField(source, 'sendFailures')),
    wsBufferedAmountPeak: safeMeasurementDuration(readMeasurementField(source, 'wsBufferedAmountPeak')),
    translatedAudioChunks: safeMeasurementCount(readMeasurementField(source, 'translatedAudioChunks')),
    outputQueuePeakMs: safeMeasurementDuration(readMeasurementField(source, 'outputQueuePeakMs')),
    outputSafetyDrops: safeMeasurementCount(readMeasurementField(source, 'outputSafetyDrops')),
    underruns: safeMeasurementCount(readMeasurementField(source, 'underruns')),
    underrunSamples: safeMeasurementCount(readMeasurementField(source, 'underrunSamples')),
    underrunDurationMs: safeMeasurementDuration(readMeasurementField(source, 'underrunDurationMs')),
    interruptions: safeMeasurementCount(readMeasurementField(source, 'interruptions')),
    firstInputToFirstTranslatedAudioMs: safeMeasurementNullableDuration(
      readMeasurementField(source, 'firstInputToFirstTranslatedAudioMs'),
    ),
    firstInputToPlaybackAcceptedMs: safeMeasurementNullableDuration(
      readMeasurementField(source, 'firstInputToPlaybackAcceptedMs'),
    ),
    terminalCategory: typeof terminalCategory === 'string'
      && safeProviderDiagnosticTokenPattern.test(terminalCategory)
      ? terminalCategory
      : null,
    playbackAccepted: readMeasurementField(source, 'playbackAccepted') === true,
  };
}

/**
 * Re-sanitize a measurement summary received from another context.
 * Unknown fields and unsafe values are discarded by the fresh DTO builder.
 * @param {unknown} summary
 * @returns {object|null}
 */
export function sanitizeLiveDubbingMeasurementSummary(summary) {
  if (!summary || typeof summary !== 'object' || summary instanceof Error || Array.isArray(summary)) {
    return null;
  }
  return createLiveDubbingMeasurementSummary(summary);
}
