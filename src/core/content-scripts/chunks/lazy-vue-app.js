...
// Import all bundled Vue component styles (automatically generated by Vite)
// This ensures that <style scoped> from all SFC components are injected into Shadow DOM
import vueComponentStyles from '@/assets/styles/vue-components-bundle.scss?inline';
...
async function createMountPoint() {
  ...
  if (!hostElement) {
    ...
    // CRITICAL: Inject all Vue component styles (Scoped CSS)
    // This allows <style scoped> from any component to work inside Shadow DOM
    if (vueComponentStyles) {
      const componentStylesElement = document.createElement('style');
      componentStylesElement.id = 'vue-sfc-styles';
      componentStylesElement.textContent = vueComponentStyles;
      shadowRoot.appendChild(componentStylesElement);
      logger.debug('Injected all Vue SFC styles into Shadow Root');
    }
