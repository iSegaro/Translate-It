...
- **Ctrl+/ and inline icon use the same behavioral contract.** Both funnel into `translateFieldViaSmartHandler({ text, target, selectionRange, sourceSnapshot })` (icon manager and shortcut handler) before applying results. An active `input`/`textarea` selection defines the source: the exact selected substring is captured with its `{ start, end }` range plus an explicit scope descriptor at request time. No selection means full-field behavior (full value, explicit full scope). The single canonical identity field is `expectedSourceText` (selected substring for selection scope, full value for full scope).
...
- **Cursor/selection must not be corrupted.** Replacement goes through via `range`/selectionStart/End and re-setSelectionRange after applying. The request-time scope is authoritative for normal `input`/`textarea` on every strategy: selection scope replaces only the captured range and full scope forces a whole-value replace, so a later caret/selection movement can never redirect old output. The live-DOM fallback stays allowed only for legacy descriptor-absent callers. Enforcement lives centrally in the shared `framework-compat` replacement pipeline (via `applicationContext.fieldSource`); strategies hold no duplicated range/stale logic and `contentEditable` semantics are untouched.
...
- **Stale translation instance cannot overwrite newer user input.** `translateFieldViaSmartHandler` aborts on an existing request for the same target, and a pending-result guard (`STALE_DATA_THRESHOLD` timestamp) drops stale data. Source identity protects both scopes before any write on every strategy path: selection requires the live substring at `[start, end]` to still equal the captured source, and full requires the live full value to still equal it — otherwise the apply fails safely with no overwrite. Caret/selection-only movement never invalidates either scope. Selection scope validates only the captured `[start, end]` source identity, so edits outside that range stay valid while the captured substring still matches; full scope validates the complete request-time field value, so any text edit (append, delete, change) invalidates the pending apply. A present-but-malformed scope descriptor also fails closed (no write); only a fully absent descriptor (legacy callers) keeps the live-DOM fallback.
- **Selection scope is independent from Copy/Replace mode.** Selected + Replace replaces only the captured range (prefix/suffix preserved); selected + Copy copies only the selection translation with no field mutation; no selection keeps the existing full-field behavior.
